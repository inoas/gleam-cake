...
import cake/fragment/where_fragment as wf
...
fn maybe_add_where(
  ...
  wf.where_fragment_append_to_prepared_statement_as_clause(qry.where, prp_stm)
}
...
fn maybe_add_select_sql(
  ...
  case qry.select {
    ...
    _ ->
      ...
      <> stringx.map_join(
        qry.select,
        select_fragment.select_fragment_to_sql,
        " ,",
      )
  }
}
...
fn maybe_add_from_sql(
  ...
  from_fragment.from_fragment_to_sql(qry.from)
}
    ...
    _ -> {
      let order_bys =
        ...
        |> list.map(fn(order_by) {
          order_by.0
          <> " "
          <> order_by_direction_fragment.order_by_direction_fragment_to_sql(
            order_by.1,
          )
        })
