...
import cake_test.{assert_snap_tap}
...
pub fn query_fragment_snap_test() {
  ...
  stacky.trace() |> stacky.frame(0) |> stacky.gleam_module_name |> iox.dbg
  ...
  let _cats_query =
    f.table(name: "cats")
    |> s.new_from()
    |> assert_snap_tap(title: "cats_query")
    // cats_query
    // |> s.where(w.eq(
    //   w.col("name"),
    //   w.fragment(
...
//     f.table(name: "cats")
//     |> s.new_from()
//     |> assert_snap_tap(title: test_function_name <> "cats_sub_query")

//   let dogs_sub_query =
//     f.table(name: "dogs")
//     |> s.new_from()
//     |> assert_snap_tap(title: test_function_name <> "dogs_sub_query")
...
//     ])
//     |> assert_snap_tap(title: test_function_name <> "where")
...
//     |> assert_snap_tap(title: test_function_name <> "base_select_query")
...
//       ]),
//     )
//     |> assert_snap_tap(title: test_function_name <> "select_query_a")

//   let where_b =
//     w.not(w.is_not_bool(w.col("is_wild"), False))
//     |> assert_snap_tap(title: test_function_name <> "where_b")
...
//     |> s.where(where_b)
//     |> assert_snap_tap(title: test_function_name <> "select_query_b")
