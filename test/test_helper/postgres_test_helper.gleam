...
fn setup_database_default_values(conn) {
  ...
  let assert Ok(_) =
    test_data.drop_users_table_if_exists() |> postgres.execute_raw_sql(conn)
  let assert Ok(_) =
    test_data.create_users_table() |> postgres.execute_raw_sql(conn)
  let assert Ok(_) =
    test_data.insert_users_rows() |> postgres.execute_raw_sql(conn)
}
