...
pub fn create_tables_and_insert_rows(conn) {
  ...
  |> run_and_echo("drop_owners_table_if_exists", conn)

  create_owners_table()
  |> run_and_echo("create_owners_table", conn)

  insert_owners_rows()
  |> run_and_echo("insert_owners_rows", conn)

  drop_cats_table_if_exists()
  |> run_and_echo("drop_cats_table_if_exists", conn)

  create_cats_table()
  |> run_and_echo("create_cats_table", conn)

  insert_cats_rows()
  |> run_and_echo("insert_cats_rows", conn)

  drop_dogs_table_if_exists()
  |> run_and_echo("drop_dogs_table_if_exists", conn)

  create_dogs_table()
  |> run_and_echo("create_dogs_table", conn)

  insert_dogs_rows()
  |> run_and_echo("insert_dogs_rows", conn)

  drop_counters_table_if_exists()
  |> run_and_echo("drop_counters_table_if_exists", conn)

  create_counters_table()
  |> run_and_echo("create_counters_table", conn)

  insert_counters_rows()
  |> run_and_echo("insert_counters_rows", conn)
